...
ii) insert into Author values('&author_id','&author_name','&author_city','&author_country');

iii) select * from author1 where author1_id in (select author_id from catalogue1 where year>2000 and price>(select avg(price) from catalogue1) group by author1_id having count(*)>1;

iv) select author1_name from author1 a.catalogue1 e where a.author1_id=e.author1_id and book_id in (select book_id from order details 1 where quantity=(select max(quantity) from order details1);

v) update catalogue1 set price=1.1* price where publisher1_id in (select publisher1_id from publisher1 where publisher1_name='person');
